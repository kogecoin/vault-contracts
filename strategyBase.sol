...
abstract contract StrategyIronBase is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public matic_titan_path;
    ...
    {
        matic_titan_path = new address[](2);
        matic_titan_path[0] = titan;
        matic_titan_path[1] = matic;
    }
    ...
    function harvest() public override {
        ...
        if (_titanBalance > harvestCutoff){
            if (_titanBalance > 0) {
                _swapUniswapWithPath(matic_titan_path, _titanBalance.div(2));
            }
            ...
            if (_matic > 0 && _titan > 0) {
                IERC20(matic).safeApprove(currentRouter, 0);
                IERC20(matic).safeApprove(currentRouter, _matic);
                IERC20(titan).safeApprove(currentRouter, 0);
                IERC20(titan).safeApprove(currentRouter, _titan);
                ...
                IERC20(titan).safeTransfer(
                    strategist,
                    IERC20(titan).balanceOf(address(this))
                );
            }
