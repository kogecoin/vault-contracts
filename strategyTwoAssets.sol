...
abstract contract StrategyIronBase is BaseStrategyMasterChef {
    ...
    address public constant iron = 0xD86b5923F3AD7b585eD81B448170ae026c65ae9a;
    ...
    // Uniswap swap paths
    address[] public titan_iron_path;
    address[] public titan_usdc_path;
    ...
    {
        titan_iron_path = new address[](2);
        titan_iron_path[0] = titan;
        titan_iron_path[1] = iron;

        titan_usdc_path = new address[](3);
        titan_usdc_path[0] = titan;
        titan_usdc_path[1] = iron;
        titan_usdc_path[2] = usdc;
    }
    ...
    function harvest() public override {
        ...
        uint256 _titanBalance = IERC20(titan).balanceOf(address(this));
        if (_titanBalance > harvestCutoff){
            _swapUniswapWithPath(titan_iron_path, _titanBalance.div(2));
            uint256 _titanBalancePost = IERC20(titan).balanceOf(address(this));
            _swapUniswapWithPath(titan_usdc_path, _titanBalancePost);
            ...
            if (_usdc > 0 && _iron > 0) {
                ...
                IERC20(iron).safeApprove(currentRouter, 0);
                IERC20(iron).safeApprove(currentRouter, _iron);
